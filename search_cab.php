<?php
session_start();


$servername = "localhost";
$username = "root";
$password = "";
$dbname = "travelease";

$conn = new mysqli($servername, $username, $password, $dbname);


if ($conn->connect_error) {
    die("Connection failed: " . $conn->connect_error);
}


if ($_SERVER["REQUEST_METHOD"] == "POST" || isset($_SESSION['search_criteria'])) {
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        
        $_SESSION['search_criteria'] = [
            'fromCity' => $_POST['from'],
            'toCity' => $_POST['to'],
            'departureDate' => $_POST['departure'],
            'pickupTime' => $_POST['pickup-time'],
            'dropTime' => $_POST['drop-time'],
        ];
    }

    
    if (isset($_SESSION['search_criteria']['fromCity']) && isset($_SESSION['search_criteria']['toCity']) && isset($_SESSION['search_criteria']['departureDate'])) {
        $fromCity = $_SESSION['search_criteria']['fromCity'];
        $toCity = $_SESSION['search_criteria']['toCity'];
        $departureDate = $_SESSION['search_criteria']['departureDate'];

        
        $sql = "SELECT * FROM Cabs WHERE FromLocation = ? AND ToLocation = ? AND DepartureDate = ? AND Availability = 'Yes'";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sss", $fromCity, $toCity, $departureDate);
        $stmt->execute();
        $result = $stmt->get_result();
        
        
        $cabs = [];
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $cabs[] = $row;
            }
        }
    } else {
        echo "No search criteria provided!";
        exit();
    }
} else {
    echo "No search criteria provided!";
    exit();
}
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="icon" href="cab.png" type="image/x-icon">
    <title>cab Booking</title>
    <style>
        body {
            font-family: Arial, sans-serif;
            background-image: url('cab background.jpg'); 
            background-size: cover;
            background-position: cover;
            background-repeat: no-repeat;
            margin: 0;
            padding: 10px;
        }
        .container {
            max-width: 900px;
            margin: auto;
            background: white;
            padding: 20px;
            border-radius: 10px;
            box-shadow: 0px 4px 6px rgba(0, 0, 0, 0.1);
        }
        .cab-card {
            display: flex;
            align-items: center;
            border: 1px solid #ddd;
            padding: 15px;
            margin-bottom: 15px;
            border-radius: 5px;
            background-color: #fff;
        }
        .cab-image {
            flex: 0 0 120px;
            margin-right: 15px;
        }
        .cab-image img {
            width: 100%;
            height: auto;
            border-radius: 5px;
        }
        .cab-details {
            flex: 1;
        }
        .cab-details h3 {
            margin: 0;
            font-size: 20px;
        }
        .cab-details p {
            margin: 5px 0;
            color: #555;
            font-size: 14px;
        }
        .book-button {
            flex: 0 0 150px;
            text-align: center;
            margin-left: auto;
        }
        .book-button button{
            display: inline-block;
            padding: 10px 20px;
            color: white;
            background-color: #007bff;
            text-decoration: none;
            border-radius: 5px;
            font-size: 16px;
        }
        .book-button button:hover {
            background-color: #0056b3;
        }
        .no-results {
            text-align: center;
            font-size: 18px;
            color: #999;
        }
    </style>
</head>
<body>
    <div class="container">
        <h2>Search cab</h2>
        <?php if (count($cabs) > 0): ?>
            <?php foreach ($cabs as $cab): ?>
                <div class="cab-card">
                    <div class="cab-image">
                        <img src="cab4.jpg" alt="cab Image"> 
                    </div>
                    <div class="cab-details">
                        <h3>Cab Name: <?php echo $cab['CabName']; ?></h3>
                        <p><strong>From:</strong> <?php echo $cab['FromLocation']; ?></p>
                        <p><strong>To:</strong> <?php echo $cab['ToLocation']; ?></p>
                        <p><strong>Pickup Time:</strong> <?php echo $cab['PickupTime']; ?></p>
                        <p><strong>Drop Time:</strong> <?php echo $cab['DropTime']; ?></p>
                        <p><strong>Availability:</strong> <?php echo $cab['Availability']; ?></p>
                    </div>
                    <div class="book-button">
                        <button onclick="bookTicket()">Book Now</button>
                    </div>
                </div>
            <?php endforeach; ?>
        <?php else: ?>
            <p class="no-results">No buses found matching your criteria.</p>
        <?php endif; ?>
    </div>
    <script>
        function bookTicket() {
            alert("Your Cab has been booked.");
            window.location.href = "cab.php";
        }
    </script>
</body>
</html>
